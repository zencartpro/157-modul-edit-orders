...
class ot_coupon extends base
{
    ...
    function process()
    {
        ...
        if ($od_amount['total'] > 0) {
            ...
            $order->info['coupon_amount'] = zen_round($od_amount['total'], $currencies->get_decimal_places($_SESSION['currency']));
            $order->info['total'] -= $order->info['coupon_amount'];
            ...
            if ((string)$orderAmountToCompareAgainstCouponMinimum >= $coupon_details['coupon_minimum_order']) {
                ...
                switch ($coupon_details['coupon_type']) {
                    case 'S': // Free Shipping
                        ...
                        break;
                    case 'P': // percentage
                        $od_amount['total'] = $orderAmountTotal * ($coupon_details['coupon_amount'] / 100);
                    ...
                    case 'F': // Fixed amount Off
                        $od_amount['total'] = ($coupon_details['coupon_amount'] > $orderTotalDetails['orderTotal'] ?
                            $orderTotalDetails['orderTotal'] : $coupon_details['coupon_amount']) * ($orderTotalDetails['orderTotal'] > 0) * $coupon_product_count;
                        $ratio = $od_amount['total'] / $orderAmountTotal;
                        break;
                ...
                switch ($this->calculate_tax) {
                    case 'Standard':
                        ...
                        foreach ($orderTotalDetails['orderTaxGroups'] as $key => $value) {
                            ...
                            $od_amount['tax_groups'][$key] = $this_tax * $ratio;
